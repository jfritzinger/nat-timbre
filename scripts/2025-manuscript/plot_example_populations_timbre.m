...
for ind = 4
	...
	plot(f_all_oboe(ind_o(ind), :)/1000, mdB_all_oboe(ind_o(ind),:), 'LineWidth', 0.8, 'Color',"#0072BD");
	[pks, locs] = findpeaks(mdB_all_oboe(ind_o(ind),:), 'MinPeakDistance', dist);
	freqs = f(locs);
	plot(freqs/1000, pks, '--', 'LineWidth', 1, 'Color',"#0072BD");

	plot(f_all(ind_b(ind), :)/1000, mdB_all(ind_b(ind),:), 'LineWidth', 0.8, 'Color',"#D95319");
	...
	scatter(CFs/1000, VS2, scattersize, 'filled', 'MarkerEdgeColor','k', ...
		'MarkerFaceAlpha',0.5, 'MarkerFaceColor',"#D95319")
	set(gca, 'xscale', 'log')
	xticks([0.1 0.2 0.5 1 2 5 10])
	xlim([0.2 10])
	%yticks([0 100])
