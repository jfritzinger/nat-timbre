...
for irep = 1:3
	...
	% % Step 2: Use cross-validation on training set for model selection/tuning
	% Mdl_cv = fitrlinear(T_train, 'Response', 'BetaTolerance', 0.0001, ...
	% 	'Learner', 'leastsquares', 'Lambda', 'auto', 'Solver', 'lbfgs', ...
	% 	'KFold', 5, 'CrossVal', 'on', 'Regularization', 'ridge');
	% 
	% % Get cross-validated performance metrics on training data
	% pred_F0_cv = kfoldPredict(Mdl_cv);
	% cv_mse = kfoldLoss(Mdl_cv);
	% fprintf('Cross-validation MSE: %.4f\n', cv_mse);
	% 
	% % Step 3: Train final model on entire training set
	% Mdl_final = fitrlinear(T_train, 'Response', 'BetaTolerance', 0.0001, ...
	% 	'Learner', 'leastsquares', 'Lambda', 'auto', 'Solver', 'lbfgs', ...
	% 	'Regularization', 'ridge');  % No cross-validation here
	% 
...
Mdl = Mdl_final;
save(fullfile(base, 'model_comparisons', ['Pop_Rate_F0_Linear_' target '_F0s.mat']),...
	"pred_F0_test", "T", "r2", "F0s", "Mdl")

%% Fit using fitlm
%
% [trainedModel, validationRMSE] = trainRegressionModelF0(T_train);
% yfit = trainedModel.predictFcn(T_test);
% r = corrcoef(yfit, T_test.Response);
% r2 = r(1, 2)^2;
% 
% figure
% nexttile
% scatter(10.^(T_test.Response), 10.^(yfit), scattersize, 'filled', ...
% 		'MarkerFaceAlpha',0.5, 'MarkerFaceColor','k')
% set(gca, 'xscale', 'log', 'yscale', 'log')
% hold on
% plot(10.^T_test.Response, 10.^T_test.Response, 'k')
...
% residuals_final = actual - yfit;

%% Beta weight analysis

trainedModels = Mdl;  % Cell array of models (one per fold)

% Preallocate matrix for coefficients
numFeatures = size(trainedModels.Beta, 1);
...
% 
% 				% Train final model with best hyperparameters
% 				Mdl = fitrlinear(T_train, 'Response', 'BetaTolerance', 1.0e-06, ...
% 					'Learner', 'leastsquares', 'Lambda', best_lambda, 'Solver', 'lbfgs', ...
% 					'Regularization', 'ridge');
% 
% 
% 				% Test set evaluation
% 				pred_F0_test = predict(Mdl, T_test);
% 				test_mse = mean((T_test.Response - pred_F0_test).^2);
% 				fprintf('Test MSE with best lambda: %.5f\n', test_mse);
% 
% 				r = corrcoef(pred_F0_test, T_test.Response);
...
% 
% % Save accuracies
% save(fullfile(base, 'model_comparisons', ['Pop_Rate_F0_' target '_CF2.mat']), ...
% 	"accuracy", "CF_groups", "num_subset", "accuracy_all")
...
% Save accuracies
save(fullfile(base, 'model_comparisons', ['Pop_Rate_F0_' target '_MTF2.mat']), ...
	"accuracy", "MTF_names", "num_subset", "accuracy_all")
