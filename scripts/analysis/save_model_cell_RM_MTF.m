...
for isesh = 1:num_index
	...
	if isempty(params_MTF)
		...
		if isempty(data_MTF.rate)
			...
			params_MTF.mnrep = 5;
			params_MTF.dur = params_MTF.dur/1000;
			...
			if strcmp(model_type, 'SFIE')
				...
				% Run model
				AN_temp = modelAN(params_MTF, model_params); % HSR for IC input
				SFIE_temp = wrapperIC(AN_temp.an_sout, params_MTF, model_params); % SFIE output

				% Plot
				if strcmp(MTF_shape, 'BS')
					[~, rate, rate_std, MTF_shape, rate_sm] = plotMTF(params_MTF, SFIE_temp.average_ic_sout_BS, 0);
					SFIE.rate = rate;
					SFIE.rate_std = rate_std;
					R = corrcoef(data_MTF.rate,rate); % Correlation
					SFIE.R = R(1, 2);
					SFIE.R2 = R(1, 2).^2;
					SFIE.SFIE_temp = SFIE_temp;
					SFIE.MTF_shape = MTF_shape;
					SFIE.rate_sm = rate_sm;

				elseif strcmp(MTF_shape, 'BE')
					[~, rate, rate_std, MTF_shape, rate_sm] = plotMTF(params_MTF, SFIE_temp.average_ic_sout_BE, 0);
					SFIE.rate = rate;
					SFIE.rate_std = rate_std;
					R = corrcoef(data_MTF.rate,rate); % Correlation
					SFIE.R = R(1, 2);
				...
				[~, rate, rate_std, MTF_shape, rate_sm] = plotMTF(params_MTF, AN_temp.average_AN_sout, 0);
				AN.rate = rate;
			...
			elseif strcmp(model_type, 'Energy') % Energy model
				...
				stimulus = [params_MTF.stim zeros(size(params_MTF.stim,1),0.1*Fs)];
				tvals = (1:length(stimulus))/Fs;
				gamma_IF_reg = zeros(1,length(tvals));
				impaired = 0; % 0 = not impaired; 1 = 'impaired'
				pin_gamma = zeros(size(stimulus, 1), Fs*params_MTF.dur+0.1*Fs);
				for istim = 1:size(stimulus, 1)
					pin_gamma(istim,:) = gamma_filt(stimulus(istim,:),gamma_param,impaired);
				end
				pin_gamma = pin_gamma(:,1:params_MTF.dur*Fs);
				energ_out = sqrt(mean(pin_gamma.^2,2));
				[rate, rate_std, pitch] = plotNT(params_MTF, energ_out, 0);
				R_int = corrcoef(data_MTF.rate,rate);
				energy.energ_out = energ_out;
				energy.rate = rate;
				energy.rate_std = rate_std;
				energy.target = params_MTF.target;
				energy.pitch = pitch;
				energy.R = R_int(1,2);
			...
			elseif strcmp(model_type, 'Lat_Inh')
				...
				% Run model
				AN_temp = modelLateralAN(params_MTF, model_params);
				latinh_temp = modelLateralSFIE(params_MTF, model_params,...
					AN_temp.an_sout, AN_temp.an_sout_lo, AN_temp.an_sout_hi,...
					'CS_params', lm_params);

				if strcmp(MTF_shape, 'BS') || strcmp(MTF_shape, 'BE')
					[rate, rate_std, pitch] = plotMTF(params_MTF, latinh_temp.avIC, 0);
					lat_inh.rate = rate;
					lat_inh.rate_std = rate_std;
					lat_inh.target = params_MTF.target;
					lat_inh.pitch = pitch;
					R = corrcoef(data_MTF.rate,rate); % Correlation
					lat_inh.R = R(1, 2);
					lat_inh.R2 = R(1, 2).^2;
					lat_inh.PSTH = plotNT_PSTH(params_MTF, latinh_temp.ic, 0);
				else
					lat_inh.rate = [];
					lat_inh.rate_std = [];
					lat_inh.R = [];
				...
				[rate, rate_std, pitch] = plotMTF(params_MTF, AN_temp.average_AN_sout, 0);
				AN_lat_inh.rate = rate;
				AN_lat_inh.rate_std = rate_std;
				AN_lat_inh.target = params_MTF.target;
				AN_lat_inh.pitch = pitch;
				R = corrcoef(data_MTF.rate,rate);
				AN_lat_inh.R = R(1, 2);
				AN_lat_inh.R2 = R(1, 2).^2;
				AN_lat_inh.PSTH = plotNT_PSTH(params_MTF, AN_temp.an_sout, 0);
			end
		end
		if isfield(params_MTF, 'stim')
			params_MTF = rmfield(params_MTF, 'stim');
		end
	end

	% Save model
	filename = [putative '_' model_type '_RM.mat'];
	if strcmp(model_type, 'Energy')
		save(fullfile('C:\DataFiles_JBF\Nat-Timbre\data\manuscript',...
			'energy_model', filename), 'params_MTF', 'energy')
	elseif strcmp(model_type, 'SFIE')
		save(fullfile('C:\DataFiles_JBF\Nat-Timbre\data\manuscript\',...
			'SFIE_model', filename), 'params_MTF', 'SFIE', 'model_params')
	elseif strcmp(model_type, 'Lat_Inh')
		save(fullfile('C:\DataFiles_JBF\Nat-Timbre\data\manuscript\',...
			'lat_inh_model', filename), 'params_MTF', 'lat_inh', 'model_params')
	end
